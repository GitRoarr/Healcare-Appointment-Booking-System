...
export class AppointmentService {
    ...
    }async deleteAppointmentByNames(patientName: string, doctorName: string): Promise<{ success: boolean; message: string }> {
        try {
            const result = await this.appointmentModel.findOneAndDelete({ patientName, doctorName });
            if (!result) {
                return { success: false, message: 'Appointment not found for the given names' };
            }
